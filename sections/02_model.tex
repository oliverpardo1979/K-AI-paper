...
\begin{equation}
 ...
 \label{eq:AN}
\end{equation}
Write $H_t\equiv A_tN_t$ and $g_H\equiv n+\gamma_A$.
...
At each date a competitive representative producer takes the gross capital rental $R_t$, the wage $w_t$, and the AI-service price $p_{X,t}$ as given. It solves
\begin{equation}
 \max_{K_Y,L,X\geq0}
 \left\{F(K_Y,AL,X)-R_tK_Y-w_tL-p_{X,t}X\right\}.
 \label{eq:final-firm}
\end{equation}
...
\begin{equation}
 Y_t=R_tK_{Y,t}+w_tL_t+p_{X,t}X_t.
...
The AI developer owns the non-rival stock $B_t$. It rents capital at the common gross rate $R_t$, uses $K_X=X/B$ to supply inference, and uses $K_R$ in research. Given the final producer's inverse demand $p_X(X;t)$, it chooses $(X_t,K_{R,t})$ to maximize
\begin{equation}
 \int_0^\infty
 \exp\!\left(-\int_0^t r_\tau\dd\tau\right)
 \left[p_{X,t}(X_t)X_t-R_t\frac{X_t}{B_t}-R_tK_{R,t}\right]\dd t
...
\begin{equation}
 \mathcal H_t^D=p_{X,t}(X_t)X_t-R_t\frac{X_t}{B_t}
 -R_tK_{R,t}+q_t\chi(B_tK_{R,t})^\eta.
 \label{eq:developer-Hamiltonian}
\end{equation}
At an interior optimum, inference satisfies the Lerner condition
\begin{equation}
 p_{X,t}(1-\varepsilon_t)=\frac{R_t}{B_t},
 \label{eq:monopoly-FOC}
\end{equation}
and research satisfies
\begin{equation}
 q_t\chi\eta B_t(B_tK_{R,t})^{\eta-1}=R_t.
 \label{eq:research-FOC}
\end{equation}
Equivalently,
\begin{equation}
 R_tK_{R,t}=q_t\eta\dot B_t.
 \label{eq:research-payment}
\end{equation}
The envelope gain from capability is the saving in inference capital, $R_tX_t/B_t^2$. Hence the costate equation is
\begin{equation}
 \dot q_t=(r_t-\eta g_{B,t})q_t-\frac{R_tX_t}{B_t^2},
...
\begin{equation}
 \Pi_t=p_{X,t}X_t-R_t(K_{X,t}+K_{R,t}).
 \label{eq:developer-profit}
\end{equation}

\subsection{Markets and feasibility}

Capital mobility implies one gross rental rate,
\begin{equation}
 R_t=r_t+\delta.
 \label{eq:R-r}
\end{equation}
...
\begin{align}
 \frac{R_tK_{Y,t}}{Y_t}&=\alpha,
 \label{eq:payment-KY}\\
 \frac{R_tK_{X,t}}{Y_t}&=(1-\alpha)s_t(1-\varepsilon_t),
 \label{eq:payment-KX}\\
 \frac{R_tK_{R,t}}{Y_t}&=\frac{q_t\eta\dot B_t}{Y_t}.
 \label{eq:payment-KR}
\end{align}
The second identity follows by multiplying~\eqref{eq:monopoly-FOC} by $X=BK_X$. It makes the accounting distinction transparent: final producers pay $p_XX$ for a service, while the developer pays $RK_X$ for the capital used to supply that service.
...
\begin{definition}[Infinite-horizon equilibrium]
Given parameters and initial conditions $(K_0,B_0,N_0,A_0)>0$, an infinite-horizon equilibrium is a collection of locally absolutely continuous state and co-state paths
\[
