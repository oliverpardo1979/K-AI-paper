...
\begin{equation}
 ...
 u_t\equiv\frac{R_tK_{X,t}}{Y_t}
 =\theta s_t(1-\varepsilon_t),\qquad
 v_t\equiv\frac{R_tK_{R,t}}{Y_t}.
 \label{eq:uv-definitions}
\end{equation}
The conventional-capital payment share is always $\alpha$. The two additional rental shares $u_t$ and $v_t$ measure the amount of the common capital stock absorbed by inference and research, valued at the same rental rate.
...
\begin{enumerate}
 \item $B_t\to\infty$;
 \item $Y_t/H_t$, $K_t/H_t$, $C_t/H_t$, $p_{X,t}$, and $R_t$ converge to finite strictly positive limits;
...
\begin{align}
 ...
 \frac{K}{Y}\longrightarrow\frac{\alpha}{\bar R},
 \label{eq:complements-capital}\\
 \frac{C}{Y}&\longrightarrow
 1-\frac{\alpha(g_H+\delta)}{\bar R}>0.
...
\end{align}
while $RK_X/Y$ and $RK_R/Y$ converge to zero.
\end{proposition}
...
\end{align}
Prices and rental shares are
\begin{align}
 r&=\rho+g-n,\qquad R=r+\delta,
 \label{eq:unit-rR}\\
 \frac{RK_Y}{Y}&=\alpha,\qquad
 \frac{RK_X}{Y}=u=\beta^2,\qquad
 \frac{RK_R}{Y}=v,
...
Consequently,
\begin{align}
 \frac{K}{Y}&=\frac{\alpha+u+v}{R},
 \label{eq:unit-KYratio}\\
 c\equiv\frac{C}{Y}
 &=1-\frac{(g+\delta)(\alpha+u+v)}{R}>0,
 \label{eq:unit-c}\\
...
\begin{equation}
 a_Y\equiv\frac{\alpha}{R},\qquad
 a_X\equiv\frac{u}{R},\qquad
 a_R\equiv\frac{v}{R},
 \label{eq:unit-a}
\end{equation}
and
...
A bounded-capability tail is regular if $B_t\to\bar B\in(0,\infty)$; $K/H$, $C/H$, and the interior static allocation ratios converge to strictly positive finite limits; $r$ and $R$ are bounded, with $R$ bounded away from zero; and the relevant growth rates converge.
\end{definition}
...
\begin{align}
 ...
 u&\equiv\lim\frac{RK_X}{Y}=\theta^2,
 &v&\equiv\lim\frac{RK_R}{Y}
 =\frac{\eta u}{d_\infty},
 \label{eq:sub-uv}\\
...
\[
 \pi^I(B;t)\equiv
 \max_{X\geq0}\left\{p_X(X;t)X-\frac{R_tX}{B}\right\}.
\]
